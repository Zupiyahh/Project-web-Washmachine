import {
  ...
  import { Connection, DeepPartial, DeleteResult, IsNull, Repository, UpdateResult } from 'typeorm';
  ...
  export class WashmachineService extends CrudService<WashMachineEntity> {
    ...
    async CreateWashMachine({
      ...
      try {
        ...
        const user = _user.id;
        const washMachine = await queryRunner.manager
    ...
    async updateWashMachineData(id: number, washMachineData: UpadateWashMachineDto): Promise<UpdateResult> {
      return await this.repository.update(id, washMachineData);
    }
  
    async DeleteWashMachine(id: number): Promise<DeleteResult> {
